...
class TmsWizardInvoice(models.TransientModel):
    _name = 'tms.wizard.invoice.waybills'
    ...
    @api.model
    def compute_travel(self, record, lines):
        res = {}
        ...
        res['partner_shipping_id'] = record.arrival_address_id
        res['account_analytic_id'] = record.unit_id.account_analytic_id
    ...
    def make_invoices(self):
        ...
        units = records.mapped('unit_id')
        if len(units) > 1:
            raise ValidationError(_("All trips must have the same truck."))
        for record in records:
            if record.invoice_id:
                raise exceptions.ValidationError(
                    _('The record is already invoiced'))
            if record.state not in ['progress', 'done', 'closed']:
                raise exceptions.ValidationError(
                    _('The record must be progress, done or closed'))
            ...
            if active_model == 'tms.travel':
                res = self.compute_travel(record, lines)

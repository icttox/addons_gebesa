...
class FleetVehicleLogFuel(models.Model):
    ...
    def _onchange_product_id(self):
        for log in self:
            if log.product_id.type == 'product':
                log.price_unit = log.product_id.standard_price
            else:
                log.price_unit = 0.00
            log.price_subtotal = log.price_unit * log.product_qty
            log.price_total = log.price_subtotal

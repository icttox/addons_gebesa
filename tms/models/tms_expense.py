...
class TmsExpense(models.Model):
    ...
    def _compute_income_km(self):
        for rec in self:
            ...
            for travel in rec.travel_ids:
                rec.start_date = travel.date_start_real
                rec.end_date = travel.date_end_real
                # for waybill in travel.waybill_ids:
                subtotal_waybills += travel.amount_untaxed
    ...
    def _compute_travel_days(self):
        for rec in self:
            if rec.start_date and rec.end_date:
                ...
                #     str(rec.end_date), "%Y-%m-%d %H:%M:%S")
                difference = rec.end_date - rec.start_date
    ...
    def create(self, values):
        if any(travel.expense_id for travel in self.travel_ids):
            raise ValidationError(
                _('Error! El viaje asignado ya tiene una liquidación '
                  'relacionada. \nFavor de verificar.'))
        expense = super().create(values)
    ...
    def write(self, vals):
        for expense in self:
            if any(travel.expense_id and travel.expense_id.id != expense.id\
                    for travel in expense.travel_ids):
                raise ValidationError(
                    _('Error! El viaje asignado ya tiene una liquidación '
                      'relacionada. \nFavor de verificar.'))
        res = super().write(vals)
        if 'travel_ids' in vals.keys():
        ...
        for rec in self:
            ...
            if line.line_type not in (
                ...
                product_account = (
                    ...
                    if (line.product_id.
                        tms_product_category == 'negative_balance' or
    ...
    def action_confirm(self):
        for rec in self:
            ...
            rec.date_confirmed = fields.Datetime.now()
    ...
    def create_fuel_line(self, fuel_log, travel):
        for rec in self:
            if fuel_log.state not in ('confirmed', 'closed', 'cancel'):
                raise ValidationError(_(

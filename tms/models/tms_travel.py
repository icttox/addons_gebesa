...
class TmsTravel(models.Model):
    ...
    instructions_driver = fields.Text(
        string='Instrucciones al Chofer',
    )
    state = fields.Selection([
        ('draft', 'Programmed'), ('progress', 'In Progress'), ('done', 'Realized'),
    ...
    @api.depends('waybill_ids', 'waybill_ids.partner_id')
    ...
    #                 str(rec.date_start), "%Y-%m-%d %H:%M:%S")
    ...
    def _compute_travel_duration(self):
        for rec in self:
            if rec.date_start and rec.date_end:
                ...
                #     str(rec.date_start), "%Y-%m-%d %H:%M:%S")
                # end_date = datetime.strptime(str(rec.date_end), "%Y-%m-%d %H:%M:%S")
                difference = (rec.date_end - rec.date_start).total_seconds() / 60 / 60
    ...
    def _compute_travel_duration_real(self):
        for rec in self:
            if rec.date_start_real and rec.date_end_real:
                start_date = datetime.strptime(
                    str(rec.date_start_real), "%Y-%m-%d %H:%M:%S")
                end_date = datetime.strptime(
                    str(rec.date_end_real), "%Y-%m-%d %H:%M:%S")
    ...
    def action_progress(self):
        for rec in self:
            ...
            rec.state = "progress"
            rec.date_progress = fields.Datetime.now()
            # rec.date_start_real = fields.Datetime.now()
            rec.message_post(body=_('<b>Travel Dispatched.</b>'))
    ...
    def action_done(self):
        for rec in self:
            ...
            rec.message_post(body=_('<b>Travel Finished.</b>'))
    ...
    def action_cancel(self):
        for rec in self:
            ...
            rec.state = "cancel"
            rec.message_post(body=_('<b>Travel Cancelled.</b>'))
        ...
        for rec in self:
            ...
            for unit in units:
                if (unit and unit.insurance_expiration and
                        unit.insurance_expiration <= datetime.date(date)):
                    raise ValidationError(_(

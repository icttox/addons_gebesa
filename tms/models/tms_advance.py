...
class TmsAdvance(models.Model):
    ...
    def action_confirm(self):
        for rec in self:
            ...
            total = rec.currency_id._convert(rec.amount, self.env.user.currency_id, rec.operating_unit_id.company_id, fields.Date.today())
            if total > 0.0:
                ...
                rec.state = 'confirmed'
                rec.message_post(body=_('<strong>Advance confirmed.</strong>'))
    ...
    def action_cancel(self):
        for rec in self:
            ...
            rec.move_id.unlink()
            rec.state = 'cancel'
            rec.message_post(body=_('<strong>Advance cancelled.</strong>'))
    ...
    def action_cancel_draft(self):
        for rec in self:
            ...
            rec.state = 'draft'
            rec.message_post(body=_('<strong>Advance drafted.</strong>'))

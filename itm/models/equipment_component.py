...
class EquipmentComponent(models.Model):
    ...
    def write(self, vals):
        ...
        for k, v in old_equips.items():
            msg = ""
            for r in v:
                msg = msg + _("<li>A %s was removed: %s</li>" % (
        ...
        msg = ""
        for r in new_equip:
            msg = msg + _("<li>A %s was installed: %s</li>" % (
                self._description, r["name"])
            )
        note = '<div class="o_mail_notification"><ul>' + msg + "</ul></div>"
    ...
    def create(self, vals):
        ...
        mt_note = self.env.ref("mail.mt_note")
        author = self.env.user.partner_id and self.env.user.partner_id.id or False
        msg = _("""<div class="o_mail_notification"><ul><li>A new %s was installed: \
    ...
    def unlink(self):
        ...
        for k, v in equips.items():
            msg = ""
            for r in v:
                msg = msg + _("<li>A %s record was deleted: %s</li>" % (
                    self._description, r["name"])
                )
            note = '<div class="o_mail_notification"><ul>' + msg + "</ul></div>"
            Equipment.browse(k).message_post(

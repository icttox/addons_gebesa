...
class ItAccess(models.Model):
    ...
    user_inactive_id = fields.Many2one(
        'res.users',
        string='User who inactive',
    )
    inactivation_date = fields.Datetime(
        string='Inactivation date',
    )
    ...
    def write(self, vals):
        ...
        for k, v in sites.items():
            msg = ""
            for r in v:
                msg = msg + _("<li>A %s's password was updated: %s</li>" % (
                    self._description, r["name"])
                )
    ...
    def create(self, vals):
        ...
        msg = _(
            """<div class="o_mail_notification"><ul><li>A new %s was created: \
                ...
                %s</a></li></ul></div>""" % (
                res._description, res.id, res.name)
        )
        if res.site_id:
    ...
    def unlink(self):
        ...
        for k, v in sites.items():
            msg = ""
            for r in v:
                msg = msg + _("<li> %s was deleted: %s</li>" % (
                    self._description, r["name"])
                )
            note = '<div class="o_mail_notification"><ul>' + msg + "</ul></div>"
            Site.browse(k).message_post(
        ...
        for k, v in equips.items():
            msg = ""
            for r in v:
                msg = msg + _("<li> %s record was deleted: %s</li>" % (
                    self._description, r["name"])
                )
            note = '<div class="o_mail_notification"><ul>' + msg + "</ul></div>"
            Equipment.browse(k).message_post(

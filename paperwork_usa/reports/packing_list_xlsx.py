...
class PackingListXlsx(models.AbstractModel):
    _name = 'report.paperwork_usa.packing_list_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for ship in objects:
            for partner in ship.partner_ids:
                ...
                sheet.write('A13', 'PURCHASE ORDER', bg_gray_format)
                sheet.write('B13', 'SALES ORDER', bg_gray_format)
                sheet.write('C13', 'VENDOR ITEM \nNUMBER', bg_gray_format)
                sheet.write('D13', 'DESCRIPTION', bg_gray_format)
                sheet.write('E13', 'INVOICE', bg_gray_format)
                sheet.write('F13', 'KG', bg_gray_format)
                sheet.write('G13', 'QTY', bg_gray_format)
                sheet.write('H13', 'PACKAGES', bg_gray_format)
                sheet.write('I13', 'NUMBER \nBINGO', bg_gray_format)
                products = ship.with_context(
                    lang=partner.partner_id.lang).mapped(
                    'line_ids').filtered(
                    lambda l: l.partner_shipping_id.id == partner.partner_id.id)
                products = sorted(products, key=lambda l: (l.sale_order_id.name, l.product_id.name))
                ...
                for line in products:
                    ...
                    if bom.type != 'phantom':
                        ...
                        sheet.write(
                            'A' + str(i), line.sale_order_id.client_order_ref,
                            border_format)
                        sheet.write(
                            'B' + str(i), line.sale_order_id.name,
                            border_format)
                        if line.order_line_id.reference_code:
                            sheet.write(
                                'C' + str(i), line.product_id.default_code + '[' + str(line.order_line_id.reference_code) + ']', border_format)
                        else:
                            sheet.write(
                                'C' + str(i), line.product_id.default_code, border_format)
                        sheet.write(
                            'D' + str(i), line.product_id.name, border_format)
                        sheet.write('E' + str(i), invoice, border_format)
                        sheet.write('F' + str(i), pitex, border_format)
                        sheet.write('G' + str(i), line.quantity_shipped, border_format)
                        ...
                        if line.order_line_id.reference_code:
                            ...
                            sheet.write(
                                'C' + str(i), line.product_id.default_code, border_format)
                        # sheet.write(
                            # 'D' + str(i), line.product_id.default_code + '[' + str(line.sale_order_id.order_line.reference_code) + ']',
                            # border_gray_format)
                        sheet.write(
                            'D' + str(i), line.product_id.product_tmpl_id.name,
                            border_gray_format)
                        sheet.write('E' + str(i), invoice, border_gray_format)
                        sheet.write('F' + str(i), '', border_gray_format)
                        sheet.write(
                            'G' + str(i), line.quantity_shipped,
                            border_gray_format)
                        sheet.write('H' + str(i), '', border_gray_format)
                        sheet.write('I' + str(i), '', border_gray_format)
                        if lang_company != partner.partner_id.lang:
                            sheet.write(
                                'J' + str(i),
                                line.product_id.with_context(
                        ...
                        for mbl in bom.bom_line_ids:
                            ...
                            sheet.write(
                                'A' + str(i), '', border_grayl_format)
                            sheet.write(
                                'B' + str(i), '', border_grayl_format)
                            if line.order_line_id.reference_code:
                                sheet.write(
                                    'C' + str(i), mbl.product_id.default_code + '[' + str(line.order_line_id.reference_code) + ']', border_format)
                            else:
                                sheet.write(
                                    'C' + str(i), mbl.product_id.default_code, border_format)
                            # sheet.write(
                                # 'D' + str(i), mbl.product_id.default_code + '[' + str(line.sale_order_id.order_line.reference_code) +']',
                                # border_grayl_format)
                            sheet.write(
                                'D' + str(i),
                                mbl.product_id.product_tmpl_id.name,
                                border_grayl_format)
                            sheet.write('E' + str(i), '', border_grayl_format)
                            sheet.write(
                                'F' + str(i), mbl_pitex, border_grayl_format)
                            sheet.write(
                                'G' + str(i), mbl_qty, border_grayl_format)
                            sheet.write('H' + str(i), '', border_grayl_format)
                            sheet.write('I' + str(i), '', border_grayl_format)
                            if lang_company != partner.partner_id.lang:
                                sheet.write(
                                    'J' + str(i),
                ...
                sheet.write('E' + str(i), 'TOTAL', bg_black_format)
                sheet.write('F' + str(i), sum_pitex, bg_black_format)
                sheet.write('G' + str(i), sum_qty, bg_black_format)

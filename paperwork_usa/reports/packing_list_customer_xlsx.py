...
class PackingListCustomerXlsx(models.AbstractModel):
    _name = 'report.paperwork_usa.packing_list_customer_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for ship in objects:
            for order in ship.sale_ids:
                sum_pitex = 0
                sum_qty = 0
                partner_id = order.sale_id.partner_shipping_id
                sheet = workbook.add_worksheet(_(
                    'Packing List ' + str(order.sale_id.id)))
                ...
                sheet.merge_range(
                    'B6:C6', partner_id.name, font_size_format)
                ...
                addr = (partner_id.street or '') +\
                    ' ' + (partner_id.street_number or '') +\
                    '\n ' + (partner_id.street2 or '') +\
                    ' ' + (partner_id.city or '') +\
                    '\n ' + (partner_id.state_id.name or '') +\
                    ' CP ' + (str(partner_id.zip) or '')
                ...
                sheet.merge_range(
                    'A10:I11',
                    "● DELIVERY INSTRUCTIONS: " + (partner_id.comment or ''), font_size_format)
                ...
                products = ship.with_context(
                    lang=partner_id.lang).mapped(
                    'line_ids').filtered(
                    lambda l: l.sale_order_id.id == order.sale_id.id)
                ...
                for line in products:
                    ...
                    if bom.type != 'phantom':
                        ...
                        if lang_company != partner_id.lang:
                            sheet.write(
                                'J' + str(i),

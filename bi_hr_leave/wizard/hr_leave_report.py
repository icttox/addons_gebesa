...
class HrLeaveReportWiz(models.TransientModel):
    ...
    def print_hr_leave_report(self):
        ...
        if len(hr_leave.ids) > 1:
            ...
            worksheet.write_merge(0, 0, 0, 7, 'Requested Leaves', green_table_header)

            row = 2
            worksheet.write(row, 0, 'Department' or '', multi_record_center)
            worksheet.write(row, 1, 'Employee Name' or '', multi_record_center)
            worksheet.write(row, 2, 'Type' or '', multi_record_center)
            worksheet.write(row, 3, 'Description' or '', multi_record_center)
            worksheet.write(row, 4, 'Period' or '', multi_record_center)
            worksheet.write(row, 5, 'Days' or '', multi_record_center)
            worksheet.write(row, 6, 'Apply Date' or '', multi_record_center)
            worksheet.write(row, 7, 'Status' or '', multi_record_center)

            row = 3
            for data in hr_leave:
                worksheet.write(row, 0, data.employee_id.department_id.name or '', multi_record)
                worksheet.write(row, 1, data.employee_id.name or '', multi_record)
                worksheet.write(row, 2, data.holiday_status_id.name or '', multi_record)
                worksheet.write(row, 3, data.name or '', multi_record)
                worksheet.write(row, 4, str(data.request_date_from.strftime('%m-%d-%Y')) + ' To ' + str(data.request_date_to.strftime('%m-%d-%Y')) or '', multi_record)
                worksheet.write(row, 5, data.number_of_days or '', multi_record)
                worksheet.write(row, 6, str(data.create_date) or '', multi_record)
                worksheet.write(row, 7, data.state or '', multi_record)
                row += 1

...
class AttendanceDeviceUser(models.Model):
    ...
    def create_employee(self):
        ...
        res = self.env['hr.employee']
        exist_employee_barcodes = Employee.search([('barcode', 'in', self.mapped('user_id'))]).mapped('barcode')
        for r in self:
            if r.user_id in exist_employee_barcodes:
                barcode = Employee._default_random_barcode()
                exist_employee_barcodes.append(barcode)
            else:
                barcode = None
            res += Employee.create(r._prepare_employee_data(barcode))
        return res

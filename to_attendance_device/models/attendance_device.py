...
class AttendanceDevice(models.Model):
    ...
    def _get_default_attendance_states(self):
        ...
        # attendance_device_state_code_2 = self.env.ref('to_attendance_device.attendance_device_state_code_2')
        # if attendance_device_state_code_2:
        #     att_state_ids += attendance_device_state_code_2
        # attendance_device_state_code_3 = self.env.ref('to_attendance_device.attendance_device_state_code_3')
        ...
        attendance_device_state_code_6 = self.env.ref('to_attendance_device.attendance_device_state_code_6')
        if attendance_device_state_code_6:
            att_state_ids += attendance_device_state_code_6
        attendance_device_state_code_7 = self.env.ref('to_attendance_device.attendance_device_state_code_7')
        if attendance_device_state_code_7:
            att_state_ids += attendance_device_state_code_7

        attendance_device_state_code_255 = self.env.ref('to_attendance_device.attendance_device_state_code_255')
        if attendance_device_state_code_255:
            att_state_ids += attendance_device_state_code_255
    ...
    def _compute_employees(self):
        ...
        for r in self:
            r.update({
                ...
                'mapped_employee_ids': [(6, 0, r.device_user_ids.mapped('employee_id').sudo().filtered(lambda employee: employee.active == True).ids)],
                })
    ...
    def _get_data_punch(self, date_check):
        timezone = pytz.timezone(self.tz or 'UTC')
        time_check = date_check.astimezone(tz=timezone).time()
        float_check = time_check.hour + (time_check.minute / 60)
        state = self.env['attendance.state'].search([
            ('start_time', '<=', float_check),
            ('end_time', '>=', float_check)], order="start_time", limit=1)
        if state:
            return state.code
        return self.env.ref('to_attendance_device.attendance_device_state_code_7').code
    ...
    @api.multi
    def map_employee_user_geb(self):
        self._cr.execute(
            """
                UPDATE attendance_device_user AS adu SET employee_id = he.id
                FROM hr_employee AS he
                WHERE 'GEB-' || LPAD(adu.user_id, 6, '0') = he.consecutive_id
                    AND adu.employee_id IS NULL
            """)
    ...
    def action_attendance_download(self):
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                if attendance.punch not in attendance_states.keys() and attendance.punch not in (2, 3):
                ...
                if user_id:
                    ...
                    if not duplicate_attend:
                        try:
                            punch = r._get_data_punch(utc_timestamp)

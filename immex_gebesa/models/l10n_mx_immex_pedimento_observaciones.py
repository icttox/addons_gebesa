...
class L10nMxImmexPedimentoObservaciones(models.Model):
    _name = 'l10n.mx.immex.pedimento.observaciones'
    ...
    secuencia = fields.Char(
        string='Secuencia de observacion',
    )
    observaciones = fields.Char(
        string='Observaciones',
    )
    tipo_pedimento = fields.Char(
        string='Clave tipo pedimento',
    )
    ...
    def _compute_pedimento_id(self):
        for ped_obs in self:
            ped_obs.pedimento_id = self.env[
                'l10n.mx.immex.pedimento'].search([
                    ('clave_aduana', '=', ped_obs.clave_aduana),
                    ('num_pedimento', '=', ped_obs.num_pedimento),
                    ('patente', '=', ped_obs.patente)])
    ...
    def import_for_data_stage(self, lines, resumen):
        for line in lines:
            ...
            self.create({
                ...
                'clave_aduana': line[2],
                'secuencia': line[3],
                'observaciones': line[4],
                'tipo_pedimento': line[5],
                'fecha_pago_real': line[6],

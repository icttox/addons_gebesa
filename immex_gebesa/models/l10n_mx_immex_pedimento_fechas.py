...
class L10nMxImmexPedimentoFechas(models.Model):
    _name = 'l10n.mx.immex.pedimento.fechas'
    ...
    tipo_fecha = fields.Char(
        string='Clave de tipo de fecha',
    )
    ...
    def _compute_pedimento_id(self):
        for ped_fec in self:
            ped_fec.pedimento_id = self.env[
                'l10n.mx.immex.pedimento'].search([
                    ('clave_aduana', '=', ped_fec.clave_aduana),
                    ('num_pedimento', '=', ped_fec.num_pedimento),
                    ('patente', '=', ped_fec.patente)])
    ...
    def import_for_data_stage(self, lines, resumen):
        for line in lines:
            ...
            self.create({
                ...
                'clave_aduana': line[2],
                'tipo_fecha': line[3],
                'fecha_operacion': line[4],
                'fecha_validacion_pago': line[5],

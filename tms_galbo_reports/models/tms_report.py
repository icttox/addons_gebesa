...
# class TmsReportManager(models.Model):
#     _name = 'tms.report.manager'
...
#     summary = fields.Char()
#     footnotes_ids = fields.One2many('tms.report.footnote', 'manager_id')
#     company_id = fields.Many2one('res.company')
#     financial_report_id = fields.Many2one('tms.financial.html.report')

#     def add_footnote(self, text, line):
#         # import ipdb; ipdb.set_trace()
#         return self.env['tms.report.footnote'].create({'line': line, 'text': text, 'manager_id': self.id})

# class TmsReportFootnote(models.Model):
#     _name = 'tms.report.footnote'
#     _description = 'TMS Report Footnote'

#     text = fields.Char()
#     line = fields.Char(index=True)
#     manager_id = fields.Many2one('tms.report.manager')

class TmsReport(models.AbstractModel):
    _name = 'tms.report'
    _description = 'TMS Report'
    ...
    def _get_templates(self):
        # import ipdb; ipdb.set_trace()
        return {
            'main_template': 'tms_galbo_reports.tms_main_template',
            'tms_main_table_header_template': 'tms_galbo_reports.tms_main_table_header',
            'tms_line_template': 'tms_galbo_reports.tms_line_template',
            'footnotes_template': 'tms_galbo_reports.tms_footnotes_template',
            'search_template': 'tms_galbo_reports.tms_search_template',
        }
    ...
    def execute_action(self, options, params=None):
        ...
        if params.get('id'):
            # Add the id of the tms.financial.html.report.line in the action's context
    ...
    def _resolve_caret_option_document(self, model, res_id, document):
        ...
        '''
        # if model == 'tms.travel':
        #     if document == 'fleet.vehicle':
        #         return self.env[model].browse(res_id).unit_id
        #     if document == 'hr.employee':
        #         return self.env[model].browse(res_id).employee_id
        #     if document == 'res.partner':
        #         return self.env[model].browse(res_id).partner_id
    ...
    def _resolve_caret_option_view(self, target):
        ...
        '''
        if target._name == 'tms.travel':
            # if target.type in ('in_refund', 'in_invoice'):
            return 'tms.view_tms_travel_form'
            # if target.type in ('out_refund', 'out_invoice'):
            #     return 'account.invoice_form'
        if target._name == 'fleet.vehicle':
            return 'fleet.fleet_vehicle_view_form'
        if target._name == 'hr.employee':
            return 'hr.view_employee_form'
    ...
    def open_document(self, options, params=None):
        ...
        # Decode params
        model = params.get('model', 'tms.travel')
        res_id = params.get('id')
        document = params.get('object', 'fleet.vehicle')
    ...
    #     active_id = int(str(params.get('id')).split('_')[0])
    #     line = self.env['tms.financial.html.report.line'].browse(active_id)
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env.ref('tms_galbo_reports.action_tms_report_tms_vehicles').read()[0]
        options['unfolded_lines'] = ['account_%s' % (params.get('id', ''),)]
        options['unfold_all'] = False
        ctx.update({'model': 'tms.fleet.vehicle'})
    ...
    #             parent_financial_report_line = self.env['tms.financial.html.report.line'].browse(params['financial_group_line_id'])
    ...
    def _create_hierarchy(self, lines):
        ...
        for line in lines + [None]:
            ...
            # And discriminating sections defined by tms.financial.html.report.line
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        render_template = templates.get('main_template', 'tms_galbo_reports.main_template')
        if line_id is not None:
            render_template = templates.get('tms_line_template', 'tms_galbo_reports.tms_line_template')
    ...
    def get_xlsx(self, options, response):
        ...
        #write all data rows
        for enu, enume in enumerate(lines):
            level = enume.get('level')
            if enume.get('caret_options'):
                style = level_3_style
                ...
                style = level_2_style
                col1_style = 'total' in enume.get('class', '').split(' ') and level_2_col1_total_style or level_2_col1_style
            elif level == 3:
                style = level_3_style
                col1_style = 'total' in enume.get('class', '').split(' ') and level_3_col1_total_style or level_3_col1_style
            else:
                style = default_style
                col1_style = default_col1_style

            #write the first column, with a specific style to manage the indentation
            cell_type, cell_value = self._get_cell_type_value(enume)
            if cell_type == 'date':
                sheet.write_datetime(enu + y_offset, 0, cell_value, date_default_col1_style)
            else:
                sheet.write(enu + y_offset, 0, cell_value, col1_style)

            #write all the remaining cells
            for x in range(1, len(enume['columns']) + 1):
                cell_type, cell_value = self._get_cell_type_value(enume['columns'][x - 1])
                if cell_type == 'date':
                    sheet.write_datetime(enu + y_offset, x + enume.get('colspan', 1) - 1, cell_value, date_default_style)
                else:
                    sheet.write(enu + y_offset, x + enume.get('colspan', 1) - 1, cell_value, style)

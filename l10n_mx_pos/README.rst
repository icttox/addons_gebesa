...
.. image:: l10n_mx_pos/static/src/img/cases_pos.png
...
This module allows create credit note from an order created from point of sale
to avoid create a negative invoice with the returned orders. All using the same
button used to creating the invoice in the pos order. For this process we are
handling three cases:

**CASE 1:**

This case is when the order that generated the return has an invoice in open
state, when this happens the credit note is created and reconciled automatically
with invoice in the main order changing their states and pay them automatically

**CASE 2:**

This case is when the order that generated the return has an invoice in paid
state, when this happens the invoice is turned to open state, breaking the
reconcile of the payment registered. Then the credit note is created and the
previous invoice is reconciled to change its state to paid again but using the
new credit note created

**CASE 3:**

This case is when the order that generated the return has no invoice related.
When this happens no credit note is created.

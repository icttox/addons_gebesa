...
class SolicitaDescarga():
    ...
    def __generar_soapreq__(self, rfc_solicitante, fecha_inicial, fecha_final, rfc_emisor, rfc_receptor, tipo_solicitud):
        ...
        if rfc_receptor is not None:
            ...
            receptores = etree.SubElement(solicitud, 'RfcReceptores')
            receptor = etree.SubElement(receptores, 'RfcReceptor')
            receptores.set('xmlns', 'http://DescargaMasivaTerceros.sat.gob.mx')
            receptor.text = rfc_receptor
    ...
    def solicitar_descarga(
        ...
        text_response = re.sub(r'\sxmlns="[^"]+"', '', response.text, count=1)
        resp_xml = etree.fromstring(text_response)

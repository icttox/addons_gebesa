...
class VmsOrder(models.Model):
    ...
    def action_released(self):
        for order in self:
            for line in order.order_line_ids:
                if line.start_date < order.start_date:
            ...
            if order.type == 'preventive':
                ...
                if not order.report_ids:
                    raise UserError(_('Debe agregar al menos un reporte a la orden de servicio.'))
    ...
    def action_open(self):
        for rec in self:
            ...
            for line in rec.order_line_ids:
                if line.start_date < rec.start_date:
                    raise UserError(_("The start date of your task cannot be less than the scheduled start date of your order."))
                if line.end_date < line.start_date:
                    raise UserError(_("Your task's scheduled finish date cannot be less than your order's scheduled start date."))

...
class SaleOrder(models.Model):
    ...
    @api.model
    def send_email_missing_cp_partners_invoices(self):
        # import ipdb; ipdb.set_trace()

        orders = self.sudo().search([
            ('geb_invoice_status', '!=', 'total_invoice'),
            ('state', 'not in', ['draft', 'cancel', 'closed'])])

        companies = orders.mapped('company_id')
        for comp in companies:
            if comp.id not in (4, 10):
                destinatarios = self.env['ir.config_parameter'].sudo().get_param('send_alert_invoices.receivers_email_dest_company_' + str(comp.id), 'False')
                table = ''
                reco = orders.filtered(lambda c: c.company_id == comp)
                partners = reco.mapped('partner_id')
                ...
                titulo = 'Codigo postal faltante del partner para facturar con 4.0 de la Empresa: ' + comp.name
                self.send_alert(body_mail, destinatarios, titulo)

    @api.model
    def send_email_canceled_orders(self):
        today = date.today()
        ...
        table = ''
        for order in orders:
            create_date_obj = order.create_date
            fecha = create_date_obj.strftime('%d-%m-%Y')
            table += """
                <tr>
                    <td align="center" style="border-bottom: 1px solid silver; color: #000; font-size: 12px; font-family:'Arial';">%s</td>
                    <td align="center" style="border-bottom: 1px solid silver; color: #000; font-size: 12px; font-family:'Arial';">%s</td>
            ...
            """ % (
                order.name, order.partner_id.name, fecha, '{:,.2f}'.format(order.amount_total) , order.approve,)

        body_mail = self.get_html_body(table)
        destinatarios = self.env['ir.config_parameter'].sudo().get_param('send_alert.canceled_orders', 'False')
        titulo = 'Alerta de pedidos aprobados cancelados ayer'
        self.send_alert(body_mail, destinatarios, titulo)
                    ...
                    <tbody><tr>
                        <td valign="center" width="270" style="padding:5px 10px
                         5px 5px;font-size: 17px; font-family:'Arial';">
                            <b>Pedidos aprobados cancelados ayer</b>
                        </td>
            ...
            #FFFFFF repeat top /100%%; color:#fff">
                ...
                90%%; background:inherit; color:inherit">
                    <tbody><tr>
                        ...
                        <th width="6%%" style="padding:5px 10px 5px 5px; font-size: 15px; font-family:'Arial';
                        border-bottom: 10px solid silver; background-color: #a24689;"><strong>Fecha creacion del pedido</strong></th>
                        <th width="6%%" style="padding:5px 10px 5px 5px; font-size: 15px; font-family:'Arial';
                        border-bottom: 10px solid silver; background-color: #a24689;"><strong>Total pedido</strong></th>
                        <th width="6%%" style="padding:5px 10px 5px 5px; font-size: 15px; font-family:'Arial';
                        border-bottom: 10px solid silver; background-color: #a24689;"><strong>Estatus de Aprobación</strong></th>
                    </tr>
                    %s
                    </tbody>
                </table>

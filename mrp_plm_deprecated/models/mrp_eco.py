...
class MrpEcoType(models.Model):
    ...
    nb_approvals_my = fields.Integer('Waiting my Approvals',
                                     compute='_compute_nb')
    ...
    def _compute_nb(self):
        # TDE FIXME: this seems not good for performances,
        # to check (replace by read_group later on)
        MrpEco = self.env['mrp.eco']
        for eco_type in self:
            eco_type.nb_ecos = MrpEco.search_count([('type_id', '=',
                                                     eco_type.id)])
...
class MrpEcoApproval(models.Model):
    ...
    required_user_ids = fields.Many2many(
        ...
        related='approval_template_id.user_ids')
...
class MrpEcoStage(models.Model):
    ...
    type_id = fields.Many2one(
        'mrp.eco.type', 'Type',
        required=True,
        default=lambda self: self.env['mrp.eco.type'].search([], limit=1))
    approval_template_ids = fields.One2many(
        'mrp.eco.approval.template', 'stage_id', 'Approvals')
    ...
    def _compute_is_blocking(self):
        self.is_blocking = any(template.approval_type == 'mandatory' for
                               template in self.approval_template_ids)
...
class MrpEco(models.Model):
    ...
    stage_id = fields.Many2one(
        'mrp.eco.stage', 'Stage', copy=False,
        domain="[('type_id', '=', type_id)]",
        group_expand='_read_group_stage_ids',
        default=lambda self: self.env['mrp.eco.stage'].search(
    ...
    user_can_approve = fields.Boolean(
        ...
        help='Technical field to check if approval by current user required')
    ...
    def _compute_bom_change_ids(self):
        ...
        new_bom_commands = []
        old_bom_lines = dict(((line.product_id, line.product_uom,), line)
        ...
        if self.new_bom_id and self.bom_id:
            for line in self.new_bom_id.bom_line_ids:
                ...
                if old_line and tools.float_compare(
                        old_line.product_qty,
                        line.product_qty, line.product_uom.rounding) != 0:
            ...
            for key, old_line in old_bom_lines.iteritems():
                new_bom_commands += [(0, 0, {
                    ...
                    'product_uom_id': old_line.product_uom.id,
                    'old_product_qty': old_line.product_qty,
                })]
    ...
    @api.depends('routing_id', 'new_routing_id')
    def _compute_routing_change_ids(self):
        # TDE TODO: should we add workcenter logic ?
        new_routing_commands = []
        old_routing_lines = dict(((op.workcenter_id,), op)
                                 for op in self.routing_id)
        if self.new_routing_id and self.routing_id:
            for operation in self.new_routing_id.operation_ids:
                ...
                if old_op and tools.float_compare(
                        old_op.time_cycle_manual,
                        operation.time_cycle_manual, 2) != 0:
    ...
    def _compute_kanban_state(self):
        """ State of ECO is based on the state of approvals for
        the current stage. """
    ...
    def onchange_type_id(self):
        self.stage_id = self.env['mrp.eco.stage'].search([
            ('type_id', '=', self.type_id.id)], limit=1).id

    @api.model
    def create(self, vals):
        prefix = self.env['ir.sequence'].next_by_code('mrp.eco') or ''
        vals['name'] = '%s%s' % (prefix and '%s: ' % prefix or '',
                                 vals.get('name', ''))
    ...
    def write(self, vals):
        if vals.get('stage_id'):
            if any(not eco.allow_change_stage for eco in self):
                raise UserError(_('You cannot change the stage,'
            ...
            if has_blocking_stages:
                raise UserError(_('You cannot change the stage,'
                                  ' as approvals are required in the process.')
                                )
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        ...
        if self._context.get('default_type_id'):
            search_domain = [
                ('type_id', '=', self._context['default_type_id'])]
    ...
    def message_post(self, **kwargs):
        ...
        if message.message_type == 'comment' and message.author_id == self.env.user.partner_id:
            for eco in self:
                for approval in eco.approval_ids.filtered(
                        lambda app: app.template_stage_id == self.stage_id and app.status == 'none'):
    ...
    def approve(self):
        for eco in self:
            for approval in eco.approval_ids.filtered(
                    lambda app: app.template_stage_id == self.stage_id and app.approval_template_id.approval_type in ('mandatory', 'optional')):
                if self.env.user in approval.approval_template_id.user_ids:
                    approval.write({
                        'status': 'approved',
                        'user_id': self.env.uid
    ...
    #             'mrp.mrp_message_view_form_embedded_bom').id,
    ...
    #             'default_message':
    #                 '%s is updated or might be soon.' % self.name
    #         },
    #         'target': 'new',
    #     }
...
class MrpEcoBomChange(models.Model):
    ...
    eco_id = fields.Many2one('mrp.eco', 'Engineering Change',
                             ondelete='cascade', required=True)
    ...
    upd_product_qty = fields.Float('Quantity',
                                   compute='_compute_upd_product_qty',
                                   store=True)
...
class MrpEcoRoutingChange(models.Model):
    ...
    upd_time_cycle_manual = fields.Float(
        'Manual Duration Change',
        compute='_compute_upd_time_cycle_manual',
        store=True)

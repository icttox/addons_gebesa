...
class AccountMoveLine(models.Model):
    ...
    def check_full_reconcile(self):
        """
        This method check if a move is totally reconciled and if we need to
        create exchange rate entries for the move.
        In case exchange rate entries needs to be created, one will be
        created per currency present.
        In case of full reconciliation, all moves belonging to the
        reconciliation will belong to the same account_full_reconcile object.
        """
        ...
        # If we have multiple currency,
        # we can only base ourselve on debit-credit
        # to see if it is fully reconciled
        currency = set(
        ...
        for aml in amls:
            ...
            if not aml.amount_currency and currency and aml.journal_id.id != aml.company_id.currency_exchange_journal_id.id:
                multiple_currency = True
        ...
        # 1) There are multiple currency different than company currency,
        # in that case we check using debit-credit
        ...
        if (
            ...
            # mark the reference of the full reconciliation on the exchange
            # rate entries and on the entries

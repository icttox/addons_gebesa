...
class SqlExecuter(models.TransientModel):
    _name = "sql.executer"
    _description = "Sql Executer"
    ...
    def execute_query(self):
        for run in self:
            fileformat = os.path.splitext(run.filename)[-1][1:].lower()
            ...
            for row in pycompat.imap(sheet.row, range(1, nrows)):
                ...
                for col in row:
                    args_list += (col.value,)
                self.env.cr.execute(run.query, args_list)
